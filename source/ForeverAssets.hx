...
class ForeverAssets
{
			...
			baseLibrary:String):FlxSprite
	{
		...
		{
			/*case "fever":
				rating.frames = Paths.getSparrowAtlas(ForeverTools.returnSkinAsset('ratings', assetModifier, changeableSkin, baseLibrary));
				switch(Timings.judgementsMap.get(asset)[0]) {
					case 0:
						rating.animation.addByPrefix("base", "sick", 24, false);
					case 1:
						rating.animation.addByPrefix("base", "good", 24, false);
					case 2:
						rating.animation.addByPrefix("base", "bad", 24, false);
					case 3:
						rating.animation.addByPrefix("base", "shit", 24, false);
					case 4:
						rating.animation.addByPrefix("base", "", 24, false);
				}
	...
	public static function generateUIArrows(x:Float, y:Float, ?staticArrowType:Int = 0, assetModifier:String):UIStaticArrow
	{
		...
		switch (assetModifier)
		{
			...
			/*case 'fever':
				...
				newStaticArrow.frames = Paths.getSparrowAtlas(ForeverTools.returnSkinAsset('$framesArgument', assetModifier, Init.trueSettings.get("Note Skin"), 'noteskins/notes'));
