 ...
 * @author larsiusprime
 */
class FNFDropUICustom extends FlxUIGroup implements IFlxUIWidget implements IFlxUIClickable implements IHasParams
{
	...
	public function new(X:Float = 0, Y:Float = 0, DataList:Array<StrNameLabel>, ?Callback:String->Void, ?Header:FlxUIDropDownHeader,
			?DropPanel:FlxUI9SliceSprite, ?ButtonList:Array<FlxUIButton>, ?UIControlCallback:Bool->FNFDropUICustom->Void)
	{
		super(X, Y);
		callback = Callback;
		header = Header;
	...
	/*public function setUIControlCallback(UIControlCallback:Bool->FNFDropUICustom->Void):Void {
		_ui_control_callback = UIControlCallback;
	}*/
