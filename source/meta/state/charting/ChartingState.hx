...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create() {
		...
		cursorNormal = new FlxSprite(0, 0).loadGraphic(Paths.image('menus/cursor/default'));
		cursorAdd = new FlxSprite(0, 0).loadGraphic(Paths.image('menus/cursor/add'));

		FlxG.mouse.visible = true;
		FlxG.mouse.load(cursorNormal.pixels, 1);
		FlxG.save.bind('pulseEngine', 'rei');
	...
	override function update(elapsed:Float) {
		...
		if (FlxG.mouse.justPressed) {
			FlxG.sound.play(Paths.sound('system/cursor/down'));
			...
			curRenderedNotes.forEach(function(note:Note) {
				if (FlxG.mouse.overlaps(note)) {
					FlxG.sound.play(Paths.sound('system/cursor/erase'));
					deleteNote(note);
				}
			});
		}
		...
		if (FlxG.mouse.justReleased) {
			FlxG.sound.play(Paths.sound('system/cursor/up'));
		}
		...
		if (FlxG.mouse.x > gridBG.x
			...
			&& FlxG.mouse.y > gridBG.y
			&& FlxG.mouse.y < gridBG.y + (GRID_SIZE * _song.notes[curSection].lengthInSteps)) {
			FlxG.mouse.load(cursorAdd.pixels, 1, Std.int(-cursorAdd.width / 2), Std.int(-cursorAdd.height / 2));
