...
import gameObjects.userInterface.AttachedSprite;
import gameObjects.userInterface.StrumNote;
import gameObjects.userInterface.notes.Note;
import gameObjects.userInterface.HealthIcon;
import gameObjects.userInterface.FlxUIDropDownMenuCustom;
import meta.data.Song;
import meta.data.dependency.Prompt;
...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Change Scroll Speed', "Value 1: Define a number for scroll\nValue 2: Define a time for scroll"]
	];
	...
	**/
	...
	override function create()
	{
		...
		var eventIcon:FlxSprite = new FlxSprite(-GRID_SIZE - 5, -90).loadGraphic(Paths.image('menus/chart/event_icon'));
	...
	function addSongUI():Void
	{
		...
		tempMap.clear();
		var stageFile:Array<String> = ["stage"];//CoolUtil.coolTextFile(Paths.txt('stageList'));
		...
		var skin = PlayState.SONG.noteSkin;
		if(skin == null) skin = '';
		...
		var reloadNotesButton:FlxButton = new FlxButton(noteSplashesInputText.x + 5, noteSplashesInputText.y + 20, 'Change Notes', function() {
			_song.noteSkin = noteSkinInputText.text;
			updateGrid();
		});
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			{
				_song.notes[curSec].sectionBeats = Math.round(nums.value);
				reloadGridLayer();
			}
	...
	*/
	...
	function updateHeads():Void
	{
		...
		if (_song.notes[curSec].mustHitSection)
		{
			...
			if (_song.notes[curSec].gfSection) leftIcon.updateIcon('gf');
		}
		else
		{
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		var note:Note = Note.returnDefaultNote("base", daStrumTime, daNoteInfo % 4, 0, 0);
	...
	function loadJson(song:String):Void
	{
		...
		if (CoolUtil.difficultyFromNumber(PlayState.storyDifficulty) != CoolUtil.defaultDifficulty) {
			if(CoolUtil.difficultyFromNumber(PlayState.storyDifficulty) == null){
				PlayState.SONG = Song.loadFromJson(song.toLowerCase(), song.toLowerCase());
			}else{
				PlayState.SONG = Song.loadFromJson(song.toLowerCase() + "-" + CoolUtil.difficultyFromNumber(PlayState.storyDifficulty), song.toLowerCase());
			}
		}else{
		PlayState.SONG = Song.loadFromJson(song.toLowerCase(), song.toLowerCase());
		}
